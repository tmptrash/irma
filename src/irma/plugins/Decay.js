...
class Decay {
    ...
    _getNearPos(org) {
        ...
        let offset = startOffs + DIR[Math.floor(org.code[0] * 8)] * Math.floor(Math.random() * Config.molDecayDistance);
        if (offset < 0 || offset > MAX_OFFS || this._world.getOrgIdx(offset) > -1) {
            offset = startOffs + DIR[Math.floor(org.code[0] * 8)] * Math.floor(Math.random() * Config.molDecayDistance);
            if (offset < 0 || offset > MAX_OFFS || this._world.getOrgIdx(offset) > -1) {
                offset = startOffs + DIR[Math.floor(org.code[0] * 8)] * Math.floor(Math.random() * Config.molDecayDistance);
            }
        }
