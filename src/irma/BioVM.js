...
class BioVM extends VM {
    ...
    constructor(options) {
        ...
        //
        if (Config.LUCAS.length + 1 + BioVM._orgsMolsAmount() > WIDTH * HEIGHT - 1) {throw Error('Amount of molecules and organisms is greater then amount of dots in a world. Decrease "molAmount" and "LUCAS.length" configs')}
        this.addOrgs();
        this.addMols();
    }
        ...
        switch (cmd) {
            ...
            case ASM: {
                ...
                loop: for (let i = Math.max(0, idx0), till = Math.min(code.length - 1, idx1); i <= till; i++) {
                    ...
                    (code[idx01] | MASK8) && (len += (this._molLastOffs(code, idx01) - idx01));
                    code[idx01] |= MASK8;
                    //
